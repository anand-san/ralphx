...
function buildPrompt(input: AgentInput): string {
  const retryContext = input.failureContext
    ? [
        ...
        ``,
        `  IMPORTANT: The previous agent may have left partial changes in the working tree.`,
        `  Before starting, run \`git status\` and \`git diff\` to check for existing work.`,
        `  If changes exist, review them and CONTINUE from where the previous agent left off.`,
