...
function buildPrompt(input: AgentInput): string {
  ...
  return [
    ...
    "",
    `## Decision Logic`,
    `- **dispatch_agent**: Task is ready to proceed or needs a different agent than current to proceed further (in case of issues). Set agentId to the agent that should handle it and provide specific agentContext instructions.`,
    `- **retry_task**: Previous attempt failed but the approach is still valid. Provide rationale explaining what to do differently. Agent will be re-dispatched but will lose older context.`,
